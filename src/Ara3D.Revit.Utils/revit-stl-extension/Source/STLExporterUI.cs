...
namespace BIM.STLExport
{
   public class STLExporterUI : IExternalApplication
   {
      ...
      Result IExternalApplication.OnStartup(UIControlledApplication application)
      {
         try
         {
            var str = "STL Exporter";
            var panel = application.CreateRibbonPanel(str);
            var directoryName = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            var data = new PushButtonData("STL Exporter for Revit", "STL Exporter for Revit", directoryName + @"\STLExport.dll", "BIM.STLExport.STLExportCommand");
            ...
            var help = new ContextualHelp(ContextualHelpType.ChmFile, directoryName + @"\Resources\ADSKSTLExporterHelp.htm");
      ...
      {
         var decoder = new PngBitmapDecoder(Assembly.GetExecutingAssembly().GetManifestResourceStream(imageResourceName), BitmapCreateOptions.PreservePixelFormat, BitmapCacheOption.Default);
